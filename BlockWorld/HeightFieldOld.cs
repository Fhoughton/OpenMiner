...
// Type: StudioForge.BlockWorld.HeightFieldOld
...
namespace StudioForge.BlockWorld
{
  public class HeightFieldOld
  {
    ...
    public HeightFieldOld(int sizeX, int sizeZ)
    {
      this.sizeX = sizeX;
      this.sizeZ = sizeZ;
    ...
    public ushort[] BuildHeightMapUShort(float maxHeight, float upperBound)
    {
      ushort[] numArray = new ushort[this.field.Length];
      for (int index = 0; index < this.field.Length; ++index)
      {
        float num = this.field[index] * maxHeight;
        numArray[index] = (ushort) ((double) upperBound + ((double) maxHeight - (double) num));
      }
      return numArray;
    }
    ...
    {
      this.random = new PcgRandom(seed);
      List<HeightFieldOld.ThreadData> threadDataList = new List<HeightFieldOld.ThreadData>();
      int num1 = 6;
      ...
      for (int index = 0; index < num1; ++index)
      {
        ...
        if (num3 > 0)
        {
          HeightFieldOld.ThreadData threadData = new HeightFieldOld.ThreadData()
          {
            Iterations = num3,
    ...
    {
      HeightFieldOld.ThreadData threadData = data as HeightFieldOld.ThreadData;
